...
部署新版客户端前应部署该接口。旧服务器返回 404 或策略不可获取时，客户端隐藏撤回命令，不猜测期限。

## 15. 权限逻辑

菜单根据消息类型、消息是否实际发送成功、发送者、当前账号、群成员列表角色、文件状态、选区和服务器期限计算。

撤回同时校验已发送的服务端消息 ID、当前发送者身份、非 FAILED/SENDING/RECALLED、服务器时钟与期限；执行时再检查，最终由服务器决定。服务端限制仍为两分钟。
...
- `npm run build:desktop`：通过，生成 `release/ArtTalk-Setup-0.0.18-x64.exe`，97,542,335 字节。
...
- Vitest：最终 `npm test` 全量运行，38 个文件、295 项测试全部通过，包含新增的 P2P 文件菜单测试。
- `node scripts/test-context-menu-ui.mjs`：28 项真实 Electron/Chromium + Vue 检查通过，使用独立假数据和替身剪贴板，不连接真实服务器或修改系统剪贴板。
...
7. **发布验收**：先部署策略 API，再用两台桌面客户端验证回复字段、双方撤回提示、真实图片剪贴板、P2P 文件重传与转发、断网恢复、换账号隔离。未提供真实凭据，本次没有替用户进行线上发送或服务器部署。
