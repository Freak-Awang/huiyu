/**
 * 文件哈希计算 Web Worker
 *
 * 在独立线程中计算文件的 SHA-256 哈希值，避免阻塞主线程 UI。
 * 使用增量哈希算法（IncrementalSha256）分块读取文件，支持大文件处理。
 * 计算过程中通过 postMessage 回传进度，完成后返回十六进制哈希字符串。
 * 
 * 核心用途：
 * - 文件秒传判断：服务端根据 SHA-256 判断文件是否已存在，跳过上传
 * - 分片上传校验：每个分片上传完成后与完整哈希比对
 */
/// <reference lib="webworker" />

/** SHA-256 常量表 K（前 64 个质数的立方根小数部分） */
const SHA256_K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
])

function rotateRight(value: number, shift: number) {
  return (value >>> shift) | (value << (32 - shift))
}

/** 增量 SHA-256 实现，支持流式更新数据，避免大文件一次性加载到内存 */
class IncrementalSha256 {
  /** 初始哈希状态（前 8 个质数的平方根小数部分） */
  private readonly state = new Uint32Array([
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
  ])
  private readonly buffer = new Uint8Array(64)
  private readonly words = new Uint32Array(64)
  private bufferLength = 0
  private bytesHashed = 0

  /** 追加数据块到哈希计算 */
  update(input: Uint8Array) {
    this.bytesHashed += input.length
    let offset = 0
    if (this.bufferLength) {
      const needed = Math.min(64 - this.bufferLength, input.length)
      this.buffer.set(input.subarray(0, needed), this.bufferLength)
      this.bufferLength += needed
      offset += needed
      if (this.bufferLength === 64) {
        this.compress(this.buffer)
        this.bufferLength = 0
      }
    }
    while (offset + 64 <= input.length) {
      this.compress(input.subarray(offset, offset + 64))
      offset += 64
    }
    if (offset < input.length) {
      this.buffer.set(input.subarray(offset), 0)
      this.bufferLength = input.length - offset
    }
  }

  /** 完成哈希计算，返回十六进制字符串 */
  digestHex() {
    const finalLength = this.bufferLength < 56 ? 64 : 128
    const finalBlock = new Uint8Array(finalLength)
    finalBlock.set(this.buffer.subarray(0, this.bufferLength))
    finalBlock[this.bufferLength] = 0x80 // 填充 1 位 + 补零
    const bitLength = this.bytesHashed * 8
    const high = Math.floor(bitLength / 0x100000000)
    const low = bitLength >>> 0
    const view = new DataView(finalBlock.buffer)
    view.setUint32(finalLength - 8, high, false) // 大端序写入消息长度高位
    view.setUint32(finalLength - 4, low, false)  // 大端序写入消息长度低位
    for (let offset = 0; offset < finalLength; offset += 64) {
      this.compress(finalBlock.subarray(offset, offset + 64))
    }
    return Array.from(this.state, (value) => value.toString(16).padStart(8, '0')).join('')
  }

  /** SHA-256 压缩函数：对 64 字节块执行 64 轮迭代 */
  private compress(chunk: Uint8Array) {
    const view = new DataView(chunk.buffer, chunk.byteOffset, chunk.byteLength)
    for (let index = 0; index < 16; index += 1) this.words[index] = view.getUint32(index * 4, false)
    for (let index = 16; index < 64; index += 1) {
      const x = this.words[index - 15]
      const y = this.words[index - 2]
      const s0 = rotateRight(x, 7) ^ rotateRight(x, 18) ^ (x >>> 3)
      const s1 = rotateRight(y, 17) ^ rotateRight(y, 19) ^ (y >>> 10)
      this.words[index] = (this.words[index - 16] + s0 + this.words[index - 7] + s1) >>> 0
    }
    let [a, b, c, d, e, f, g, h] = this.state
    for (let index = 0; index < 64; index += 1) {
      const s1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25)
      const choice = (e & f) ^ (~e & g)
      const t1 = (h + s1 + choice + SHA256_K[index] + this.words[index]) >>> 0
      const s0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22)
      const majority = (a & b) ^ (a & c) ^ (b & c)
      const t2 = (s0 + majority) >>> 0
      h = g; g = f; f = e; e = (d + t1) >>> 0
      d = c; c = b; b = a; a = (t1 + t2) >>> 0
    }
    this.state[0] = (this.state[0] + a) >>> 0
    this.state[1] = (this.state[1] + b) >>> 0
    this.state[2] = (this.state[2] + c) >>> 0
    this.state[3] = (this.state[3] + d) >>> 0
    this.state[4] = (this.state[4] + e) >>> 0
    this.state[5] = (this.state[5] + f) >>> 0
    this.state[6] = (this.state[6] + g) >>> 0
    this.state[7] = (this.state[7] + h) >>> 0
  }
}

/**
 * Worker 消息处理入口
 * 接收 { file: File, chunkSize?: number }，分块计算 SHA-256 哈希
 * 每处理一个分块回传 { type: 'progress', loaded, total }
 * 完成后回传 { type: 'complete', sha256 }
 * 出错时回传 { type: 'error', message }
 */
self.onmessage = async (event: MessageEvent<{ file: File; chunkSize?: number }>) => {
  try {
    const file = event.data.file
    const chunkSize = Math.max(1024 * 1024, event.data.chunkSize || 8 * 1024 * 1024)
    const sha256 = new IncrementalSha256()
    for (let offset = 0; offset < file.size; offset += chunkSize) {
      const end = Math.min(file.size, offset + chunkSize)
      sha256.update(new Uint8Array(await file.slice(offset, end).arrayBuffer()))
      self.postMessage({ type: 'progress', loaded: end, total: file.size })
    }
    self.postMessage({ type: 'complete', sha256: sha256.digestHex() })
  } catch (error) {
    self.postMessage({ type: 'error', message: error instanceof Error ? error.message : String(error) })
  }
}

export {}
